...
namespace MvcApplication2.Controllers
{
    public class CursoController : Controller
    {
        ...
        {
            var nombrecurso = from s in db.nombreCursoes
                              where s.CursoId == id
                              select s;

            return Json(new SelectList(nombrecurso.ToArray(), "nombreCursoId", "nombre"), JsonRequestBehavior.AllowGet);
        ...
        public IList<nombreCurso> GetState(int CursoId)
            {
                return db.nombreCursoes.Where(m => m.CursoId == CursoId).ToList();
            }



        public JsonResult LoadClassesByCursoId(string tipo)
        {
            var nombrecursoList = this.GetState(Convert.ToInt32(tipo));
            var nombrecursodata = nombrecursoList.Select(m => new SelectListItem()
                {
                    Text = m.nombre,
                    Value = m.CursoId.ToString(),
                });

            return Json(nombrecursodata, JsonRequestBehavior.AllowGet);
        }
